...
type Member = {
  ...
  pending_workspaces: CustomerWorkspace[];
};
...
export function UsersPage() {
  ...
  async function load() {
    const { data, error } = await supabase.rpc('list_organisation_staff');
  ...
  return <div className="max-w-5xl mx-auto space-y-6">
    ...
    <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
      ...
      {members.map(member => {
        const displayedWorkspaces = member.status === 'invited' ? member.pending_workspaces : member.workspaces;
        return <div key={member.membership_id} data-testid="organisation-staff-row" data-user-email={member.email} className="p-5 border-b last:border-b-0 flex flex-col sm:flex-row sm:items-center gap-4"><div className="flex-1"><p className="font-semibold text-slate-900">{member.full_name || member.email}</p><p className="text-xs text-slate-500">{member.email} · <span data-testid="staff-status">{member.status}</span>{member.status === 'invited' ? ' · inactive' : ''}</p><div className="flex flex-wrap gap-1 mt-2">{displayedWorkspaces.map(item => <span key={item} data-testid="staff-workspace" className="text-[10px] bg-slate-100 text-slate-600 rounded-full px-2 py-1">{OPTIONS.find(option => option.id === item)?.label}</span>)}</div></div>{member.status !== 'invited' && <div className="flex gap-2"><button onClick={() => { setEditing(member); setEditAccess(member.workspaces); }} className="px-3 py-2 border border-slate-300 rounded-lg text-xs font-medium"><Shield className="w-3.5 h-3.5 inline mr-1" />Access</button>{member.status === 'inactive' ? <button disabled={busy} onClick={() => setStatus(member, 'active')} className="px-3 py-2 border border-emerald-300 text-emerald-700 rounded-lg text-xs font-medium"><UserCheck className="w-3.5 h-3.5 inline mr-1" />Reactivate</button> : <button disabled={busy} onClick={() => setStatus(member, 'inactive')} className="px-3 py-2 border border-red-200 text-red-600 rounded-lg text-xs font-medium"><UserX className="w-3.5 h-3.5 inline mr-1" />Deactivate</button>}</div>}</div>;
      })}
    </div>
