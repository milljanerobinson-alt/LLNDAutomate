/*
# EWO-047 R2 Fix — Type-Safe Cast for Repository Drift Check

## Root Cause
In public.start_governed_execution, step 9 (repository config drift check),
the predicate compares:
  github_repository_config.project_id  (text)
to
  v_ewo.project_id                      (uuid)

PostgreSQL has no implicit cast between text and uuid, so the RPC raises:
  operator does not exist: text = uuid  (SQLSTATE 42883)

## Fix
Add an explicit cast on the uuid operand:
  WHERE project_id = v_ewo.project_id::text

This is safe because v_ewo.project_id originates from
engineering_work_orders.project_id (a uuid column), not from user input.
The ::text cast produces the canonical UUID string representation, which is
exactly how github_repository_config.project_id stores the value.

## Scope
- Replaces ONLY the public.start_governed_execution function definition.
- Preserves all existing gates, transaction behaviour, lifecycle rules,
  audit writes, and rollback behaviour.
- Does not alter either column type.
- Does not add broad implicit casts.
- complete_execution_start and rollback_execution_start are unchanged.
*/
...
BEGIN
  ...
  --    github_repository_config.project_id is text;
 v_ewo.project_id is uuid.
  --    Explicit cast prevents "operator does not exist: text = uuid".
...
-- Re-grant permissions (preserved from R2)
