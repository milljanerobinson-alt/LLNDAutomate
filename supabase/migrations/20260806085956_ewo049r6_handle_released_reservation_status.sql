/*
# EWO-049R6 — Handle 'released' reservation status in governed gateway

## Purpose

The governed deletion cleanup (EWO-032R12) sets reservation status to
'released' when an EWO is deleted. When the Product Owner retries creation
with the same plan slug, the R5 RPC encounters the 'released' status and
falls to the ELSE branch, rejecting with "unrecognised status 'released'".

This migration teaches the RPC to treat 'released' reservations the same
as 'reserved' — the reservation is available for reuse. The previous EWO
was deleted through the governed cleanup, so the reservation is safe to
reuse for a new EWO creation.

## Changes

### create_canonical_ewo_governed (Overload 2)

Gate 2 is updated:
  - 'released' status is now treated identically to 'reserved'
  - The reservation is reset to 'reserved' status and made available
    for the current creation attempt
  - All other status handling (consumed with live EWO, consumed orphan,
    consumed no ewo_id, reserved) remains unchanged
...
BEGIN
  ...
  -- ─── Gate 2: Resolve reservation (SAFE ORPHAN RECOVERY + RELEASED REUSE) ───
 ...
 -- do nothing, reservation is ready
      ...
      ELSIF v_reservation.status = 'released' THEN
        -- EWO-049R6: 'released' means the previous EWO was deleted through
        -- governed cleanup. The reservation is safe to reuse for a new EWO.
        -- Reset to 'reserved' so the current creation can proceed.
        UPDATE ewo_ref_reservations
        SET status = 'reserved',
            consumed_at = NULL,
            ewo_id = NULL,
            reserved_at = now(),
            reserved_by = COALESCE(p_created_by_email, 'system'),
            reservation_context = 'governed_gateway_released_reuse',
            correlation_id = v_audit_ref
        WHERE ewo_ref = p_reserved_ewo_ref;
